...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		FlxG.sound.pause();
		// Stage
		var bg:BGSprite = new BGSprite('stageback', -300, -185, 0.8, 0.75);
		add(bg);

		// Characters
		gf = new Character(500, 150, 'aquino-gf');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		
		boyfriend = new Character(900, 50, 'bf', true);
		boyfriend.x += boyfriend.positionArray[0];
		boyfriend.y += boyfriend.positionArray[1];

		dad = new Character(250, 100, 'locochon', false);
		...
		beatText = new Alphabet(0, 0, '¡Beat!', true);
		beatText.scaleX = 0.6;
		beatText.scaleY = 0.6;
		beatText.x += 260;
		beatText.alpha = 0;
		...
		if(controls.BACK)
		{
			...
			if(options.OptionsState.onPlayState)
			{
				if(ClientPrefs.pauseMusic != 'Desactivado')
					FlxG.sound.playMusic(Paths.music(Paths.formatToSongPath(ClientPrefs.pauseMusic)), 1, true);
				if(ClientPrefs.pauseMusic == 'Song Inst.')
					FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 1, true);
			}else{
				if (ClientPrefs.titleVer == 'DLC 1'){
					FlxG.sound.playMusic(Paths.music('freakyMenuDM1'), 1, true);
				}else if (ClientPrefs.titleVer == 'DLC 2'){
					FlxG.sound.playMusic(Paths.music('freakyMenuDM2'), 1, true);
				}else if (ClientPrefs.titleVer == 'DLC 3'){
					FlxG.sound.playMusic(Paths.music('freakyMenuDM3'), 1, true);
				}else{
					FlxG.sound.playMusic(Paths.music('freakyMenu'), 1, true);
				}
			}
	...
	function updateMode()
	{
		...
		if(onComboMenu)
			changeModeText.text = '< Combo Offset (Presiona la tecla Aceptar para cambiar) >';
		else
			changeModeText.text = '< Note/Beat Delay (Presiona la tecla Aceptar para cambiar) >';

	...
	public function new(script:String) {
		...
		try{
			...
			if(resultStr != null && result != 0) {
				trace('Error en script Lua! ' + resultStr);
				#if windows
				lime.app.Application.current.window.alert(resultStr, 'Error en script Lua!');
				#else
				luaTrace('Error al cargar el script Lua: "$script"\n' + resultStr, true, false, FlxColor.RED);
				#end
				lua = null;
				return;
			}
		...
		trace('Archivo Lua cargado correctamente: ' + script);

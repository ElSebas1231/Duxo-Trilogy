...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	'JKLI - Mover Cámara (Presiona Shift para ir 4 veces más rápido)
	\nQ/E - Agregar/Reducir Zoom
	\nR - Resetear Cámara
	\nH - Alternar burbuja de díalogo
	\nSpace - Resetear texto';
	...
	\nR - Resetear Cámara
	\nH - Alternar Sombras
	\nWASD - Mover Offset de Animaciones Loopeadas  (Sombra Roja)
	\nArrow Keys - Mover Offset de Animaciones Idle/Terminadas (Sombra Azul)
	\nmantén Shift para ir 10 veces más rápido';
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Posición de Personaje', label: 'Posición de Personaje'},
		];
		...
		addCharacterUI();
		add(UI_mainbox);
		UI_mainbox.selected_tab_id = 'Personaje';
	...
	function addTypeUI() {
		...
		tab_group.name = "Posición de Personaje";

		leftCheckbox = new FlxUICheckBox(10, 20, null, null, "Izquierda", 100);
		...
		centerCheckbox = new FlxUICheckBox(leftCheckbox.x, leftCheckbox.y + 40, null, null, "Centrado", 100);
		...
		rightCheckbox = new FlxUICheckBox(centerCheckbox.x, centerCheckbox.y + 40, null, null, "Derecha", 100);
	...
	function addAnimationsUI() {
		...
		var addUpdateButton:FlxButton = new FlxButton(10, idleInputText.y + 30, "Añadir/Act.", function() {
		...
		var removeUpdateButton:FlxButton = new FlxButton(100, addUpdateButton.y, "Remover", function() {
			for (i in 0...character.jsonFile.animations.length) {
				var animArray:DialogueAnimArray = character.jsonFile.animations[i];
		...
		tab_group.add(new FlxText(animationInputText.x, animationInputText.y - 18, 0, 'Nombre de Anim.:'));
		tab_group.add(new FlxText(loopInputText.x, loopInputText.y - 18, 0, 'Nombre de Loop en .XML:'));
		tab_group.add(new FlxText(idleInputText.x, idleInputText.y - 18, 0, 'Nombre de Idle/Termi. en .XML:'));
	...
	function addCharacterUI() {
		...
		tab_group.add(new FlxText(10, imageInputText.y - 18, 0, 'Nombre de la imágen:'));
		tab_group.add(new FlxText(10, xStepper.y - 18, 0, 'Posición de Offset:'));
		tab_group.add(new FlxText(10, scaleStepper.y - 18, 0, 'Escala:'));
		...
		var reloadImageButton:FlxButton = new FlxButton(10, scaleStepper.y + 60, "Recargar Img.", function() {
			reloadCharacter();
		});
		
		var loadButton:FlxButton = new FlxButton(reloadImageButton.x + 100, reloadImageButton.y, "Cargar Pers.", function() {
			loadCharacter();
		});
		var saveButton:FlxButton = new FlxButton(loadButton.x, reloadImageButton.y - 25, "Guardar Pers.", function() {
			saveCharacter();
		});
	...
	function reloadCharacter() {
		...
		curAnim = 0;
		animText.text = 'Animación: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W o S para ir desplazando anims.';

		#if desktop
		// Updating Discord Rich Presence
		DiscordClient.changePresence("Editor de Personaje de Diálogo", "Editando: " + character.jsonFile.image, null, false, null, 'icon');
		#end
	}
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.SPACE && UI_mainbox.selected_tab_id == 'Personaje') {
			...
			if(UI_mainbox.selected_tab_id == 'Animaciones' && curSelectedAnim != null && character.dialogueAnimations.exists(curSelectedAnim)) {
			...
			if(FlxG.keys.justPressed.H) {
				if(UI_mainbox.selected_tab_id == 'Animaciones') {
					currentGhosts++;
			...
			if(UI_mainbox.selected_tab_id != lastTab) {
				if(UI_mainbox.selected_tab_id == 'Animaciones') {
					...
					character.playAnim(character.jsonFile.animations[curAnim].anim);
					animText.text = 'Animaciones: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W o S para ir desplazando anims.';
				}
				lastTab = UI_mainbox.selected_tab_id;
				currentGhosts = 0;
			}
			
			if(UI_mainbox.selected_tab_id == 'Personaje')
			{
				...
				if(controlAnim.contains(true))
				{
					...
					animText.text = 'Personaje: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W o S para ir desplazando anims.';
				}
			}

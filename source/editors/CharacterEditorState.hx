...
class CharacterEditorState extends MusicBeatState
{
	...
	{
		FlxG.sound.playMusic(Paths.music('breakfastMuffled'));
		...
		var tipTextArray:Array<String> = "E/Q - Agregar/Reducir Zoom de Cámara
		\nR - Reseteear Zoom
		\nJKLI - Mover Cámara
		\nW/S - Previa/Siguiente Animación
		\nSpace - Reproducir Animación
		\nFlechas del Teclado - Mover Offset
		\nT - Resetear Offset Actual
		\nMantener Shift para mover 10 veces más rápido\n".split('\n');
		...
		var tabs = [
			{name: 'Personaje', label: 'Personaje'},
			{name: 'Animaciones', label: 'Animaciones'},
		];
		...
		addCharacterUI();
		addAnimationsUI();
		UI_characterbox.selected_tab_id = 'Personaje';
	...
	function addSettingsUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Opciones";

		var check_player = new FlxUICheckBox(10, 60, null, null, "Personaje Jugable", 100);
		...
		var reloadCharacter:FlxButton = new FlxButton(140, 20, "Recargar Pers.", function()
		{
			loadChar(!check_player.checked);
			reloadCharacterDropDown();
		});

		var templateCharacter:FlxButton = new FlxButton(140, 50, "Cargar Planti.", function()
		{
		...
		tab_group.add(new FlxText(charDropDown.x, charDropDown.y - 18, 0, 'Personaje:'));
	...
	function addCharacterUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Personaje";

		imageInputText = new FlxUIInputText(15, 30, 200, 'characters/BOYFRIEND', 8);
		var reloadImage:FlxButton = new FlxButton(imageInputText.x + 210, imageInputText.y - 3, "Recargar Img.", function()
		{
		...
		var decideIconColor:FlxButton = new FlxButton(reloadImage.x, reloadImage.y + 30, "Obtener Color", function()
			{
		...
		flipXCheckBox = new FlxUICheckBox(singDurationStepper.x + 80, singDurationStepper.y, null, null, "Voltear X", 50);
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x, noAntialiasingCheckBox.y + 40, "Guardar Pers.", function() {
			saveCharacter();
		});
		...
		tab_group.add(new FlxText(15, imageInputText.y - 18, 0, 'Nombre de la Img.:'));
		tab_group.add(new FlxText(15, healthIconInputText.y - 18, 0, 'Nombre del Ícono:'));
		tab_group.add(new FlxText(15, singDurationStepper.y - 18, 0, 'Duración de Sing Anim.:'));
		tab_group.add(new FlxText(15, scaleStepper.y - 18, 0, 'Escala:'));
		tab_group.add(new FlxText(positionXStepper.x, positionXStepper.y - 18, 0, 'X/Y del Personaje:'));
		tab_group.add(new FlxText(positionCameraXStepper.x, positionCameraXStepper.y - 18, 0, 'X/Y de la Cámara:'));
		tab_group.add(new FlxText(healthColorStepperR.x, healthColorStepperR.y - 18, 0, 'R/G/B de la Barra de Vida:'));
	...
	function addAnimationsUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Animaciones";

		animationInputText = new FlxUIInputText(15, 85, 80, '', 8);
		animationNameInputText = new FlxUIInputText(animationInputText.x, animationInputText.y + 35, 150, '', 8);
		animationIndicesInputText = new FlxUIInputText(animationNameInputText.x, animationNameInputText.y + 40, 250, '', 8);
		animationNameFramerate = new FlxUINumericStepper(animationInputText.x + 170, animationInputText.y, 1, 24, 0, 240, 0);
		animationLoopCheckBox = new FlxUICheckBox(animationNameInputText.x + 170, animationNameInputText.y - 1, null, null, "¿Debería ser Loopeada?", 100);
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Añadir/Act.", function() {
			...
			genBoyOffsets();
			trace('Animación Añadida/Actualizada: ' + animationInputText.text);
		});
		...
		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 30, "Remover", function() {
			for (anim in char.animationsArray) {
				if(animationInputText.text == anim.anim) {
					...
					reloadAnimationDropDown();
					genBoyOffsets();
					trace('Animación Removida: ' + animationInputText.text);
					break;
				}
			}
		});

		tab_group.add(new FlxText(animationDropDown.x, animationDropDown.y - 18, 0, 'Animaciones:'));
		tab_group.add(new FlxText(ghostDropDown.x, ghostDropDown.y - 18, 0, 'Animación Sombra:'));
		tab_group.add(new FlxText(animationInputText.x, animationInputText.y - 18, 0, 'Nombre de la Animación:'));
		tab_group.add(new FlxText(animationNameFramerate.x, animationNameFramerate.y - 18, 0, 'Framerate:'));
		tab_group.add(new FlxText(animationNameInputText.x, animationNameInputText.y - 18, 0, 'Animación en archivo .XML/.TXT:'));
		tab_group.add(new FlxText(animationIndicesInputText.x, animationIndicesInputText.y - 18, 0, 'AVANZADO - Índices de la Animación:'));
	...
	function genBoyOffsets():Void
	{
		...
		if(dumbTexts.length < 1) {
			var text:FlxText = new FlxText(10, 38, 0, "¡ERROR! No se encontrarón animaciones.", 15);
			text.scrollFactor.set();
			text.borderSize = 1;
			dumbTexts.add(text);
	...
	function updatePresence() {
		...
		DiscordClient.changePresence("Editor de Personajes", "Editando a: " + daAnim, leHealthIcon.getCharacter(), false, null, 'icon');
		#end
	}
	...
	function onSaveComplete(_):Void
	{
		...
		_file = null;
		FlxG.log.notice("Archivo correctamente guardado.");
	}
	...
	function onSaveError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
		FlxG.log.error("Hubo un problema al guardar el archivo");
	}

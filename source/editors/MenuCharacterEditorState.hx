...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		DiscordClient.changePresence("Editor de Personajes de Menú", "Editando: " + characterFile.image, null, false, null, 'icon');
		#end
		...
		var tipText:FlxText = new FlxText(0, 540, FlxG.width,
			"Arrow Keys - Cambiar Offset (Mantener SHIFT para ir 10 más rápido)
			\nSpace - Reproducir la animación \"Start Press\" (Pers. Tipo Boyfriend)", 16);
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Tipo de Personaje', label: 'Tipo de Personaje'},
		];
		...
		var loadButton:FlxButton = new FlxButton(0, 480, "Cargar Personaje", function() {
			loadCharacter();
		});
		...
		var saveButton:FlxButton = new FlxButton(0, 480, "Guardar Personaje", function() {
			saveCharacter();
		});
	...
	function addTypeUI() {
		...
		tab_group.name = "Tipo de Personaje";
	...
	function addCharacterUI() {
		...
		flipXCheckbox = new FlxUICheckBox(10, confirmInputText.y + 30, null, null, "Voltear X", 100);
		...
		var confirmDescText = new FlxText(10, confirmInputText.y - 18, 0, 'Animación "Start Press" en el .XML:');
		tab_group.add(new FlxText(10, imageInputText.y - 18, 0, 'Nombre de la imágen:'));
		tab_group.add(new FlxText(10, idleInputText.y - 18, 0, 'Idle animation en el .XML:'));

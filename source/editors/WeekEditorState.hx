...
class WeekEditorState extends MusicBeatState
{
	...
	function addEditorBox() {
		...
		var loadWeekButton:FlxButton = new FlxButton(0, 650, "Cargar Week", function() {
			loadWeek();
		});
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 650, "Guardar Week", function() {
			saveWeek(weekFile);
		});
	...
	function addWeekUI() {
		...
		hideCheckbox = new FlxUICheckBox(10, weekFileInputText.y + 40, null, null, "¿Ocultar del Story Mode?", 100);
		...
		tab_group.add(new FlxText(opponentInputText.x, opponentInputText.y - 18, 0, 'Personajes:'));
		tab_group.add(new FlxText(backgroundInputText.x, backgroundInputText.y - 18, 0, 'Imagen de Fondo:'));
		tab_group.add(new FlxText(displayNameInputText.x, displayNameInputText.y - 18, 0, 'Nombre:'));
		tab_group.add(new FlxText(weekNameInputText.x, weekNameInputText.y - 18, 0, 'Nombre de Week (Para el Menú de Reseteo):'));
		tab_group.add(new FlxText(weekFileInputText.x, weekFileInputText.y - 18, 0, 'Archivo de la Week:'));
	...
	function addOtherUI() {
		...
		lockedCheckbox = new FlxUICheckBox(10, 30, null, null, "Empezar bloqueada", 100);
		...
		hiddenUntilUnlockCheckbox = new FlxUICheckBox(10, lockedCheckbox.y + 25, null, null, "Oculto hasta desbloquear", 110);
		...
		tab_group.add(new FlxText(weekBeforeInputText.x, weekBeforeInputText.y - 28, 0, 'Nombre de la semana que debes\nterminar para desbloquear:'));
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y - 20, 0, 'Dificultades:'));
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y + 20, 0, 'Dificultades por default son: "Easy, Normal, Hard"\nsin comillas.'));
	...
	function reloadWeekThing() {
		...
		if(isMissing) {
			...
			missingFileText.text = 'IMÁGEN FALTANTE: images/storymenu/' + assetName + '.png';
		}
		recalculateStuffPosition();

		#if desktop
		// Updating Discord Rich Presence
		DiscordClient.changePresence("Editor de Semanas", "Editando: " + weekFileName, null, false, null, 'icon');
		#end
	}
	...
	private static function onLoadComplete(_):Void
	{
		...
		if(fullPath != null) {
			...
			if(rawJson != null) {
				...
				if(loadedWeek.weekCharacters != null && loadedWeek.weekName != null) //Make sure it's really a week
				{
					...
					trace("Archivo correctamente cargado: " + cutName);
					loadError = false;
		...
		_file = null;
		#else
		trace("¡No se puede cargar el archivo! ¿No estás en una PC, verdad?");
		#end
	}
		...
		private static function onLoadCancel(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		trace("Carga del archivo cancelada.");
	}
	...
	private static function onLoadError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		trace("Ocurrió un problema al cargar el archivo");
	}
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	function addEditorBox() {
		...
		var loadWeekButton:FlxButton = new FlxButton(0, 685, "Cargar Week", function() {
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 685, "Guardar Week", function() {
	...
	function addFreeplayUI() {
		...
		var copyColor:FlxButton = new FlxButton(10, bgColorStepperR.y + 25, "Copiar Color", function() {
			Clipboard.text = bg.color.red + ',' + bg.color.green + ',' + bg.color.blue;
		});
		var pasteColor:FlxButton = new FlxButton(140, copyColor.y, "Pegar Color", function() {
		...
		tab_group.add(new FlxText(10, bgColorStepperR.y - 18, 0, 'Color de Fondo Seleccionado en R/G/B:'));
		tab_group.add(new FlxText(10, iconInputText.y - 18, 0, 'Ícono Seleccionado:'));

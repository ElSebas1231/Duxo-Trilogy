...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		var addLineText:FlxText = new FlxText(10, 10, FlxG.width - 20, 'Presiona O para eliminar la línea actual de díalogo - Presiona P para añadir otra linea de díalogo a la actual.', 8);
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Línea de Diálogo', label: 'Línea de Diálogo'},
		];
	...
	function addDialogueLineUI() {
		...
		tab_group.name = "Línea de Diálogo";
		...
		var loadButton:FlxButton = new FlxButton(20, lineInputText.y + 25, "Cargar Diálogo", function() {
			loadDialogue();
		});
		var saveButton:FlxButton = new FlxButton(loadButton.x + 120, loadButton.y, "Guardar Diál.", function() {
			saveDialogue();
		});

		tab_group.add(new FlxText(10, speedStepper.y - 18, 0, 'Intérvalo/Velocidad (ms):'));
		tab_group.add(new FlxText(10, characterInputText.y - 18, 0, 'Personaje:'));
		tab_group.add(new FlxText(10, soundInputText.y - 18, 0, 'Nombre del archivo de sonido:'));
	...
	function reloadCharacter() {
		...
		if(character.animation.curAnim != null && character.jsonFile.animations != null) {
			...
			animText.text = '¡ERROR! NO SE ENCONTRARON ANIMACIONES';
		}
	}
	...
	function reloadText(skipDialogue:Bool) {
		...
		if(rpcText == null || rpcText.length < 1) rpcText = '(Vacío)';
		if(rpcText.length < 3) rpcText += '   '; //Fixes a bug on RPC that triggers an error when the text is too short
		DiscordClient.changePresence("Editor de Diálogos", rpcText, null, false, null, 'icon');
		#end
	}
		...
		if(!blockInput) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				...
				if (ClientPrefs.titleVer == 'DLC 1'){
					FlxG.sound.playMusic(Paths.music('freakyMenuDM1'), 1);
				}else if (ClientPrefs.titleVer == 'DLC 2'){
					FlxG.sound.playMusic(Paths.music('freakyMenuDM2'), 1);
				}else if (ClientPrefs.titleVer == 'DLC 3'){
					FlxG.sound.playMusic(Paths.music('freakyMenuDM3'), 1);
				}else{
			...
			for (i in 0...controlAnim.length) {
				if(controlAnim[i] && character.jsonFile.animations.length > 0) {
					...
					animText.text = 'Animation: ' + animToPlay + ' (' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + ') - Presiona W o S para ir desplazando anims.';
				}
				if(controlText[i]) {
	...
	function changeText(add:Int = 0) {
		...
		if(leLength > 0) {
			...
			character.playAnim(character.jsonFile.animations[curAnim].anim, daText.finishedText);
			animText.text = 'Animación: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + leLength + ') - Presiona W o S para ir desplazando anims.';
		} else {
			animText.text = '¡ERROR! NO SE ENCONTRARON ANIMACIONES';
		}
		characterAnimSpeed();

		selectedText.text = 'Línea: (' + (curSelected + 1) + ' / ' + dialogueFile.dialogue.length + ') - Presiona A o D para ir a la siguiente línea';
	}

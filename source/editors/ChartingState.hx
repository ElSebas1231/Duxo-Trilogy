...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Editor de Charteo", "Charteando: " + StringTools.replace(_song.song, '-', ' '), null, false, null, 'icon');
		#end
		...
		var tabs = [
			{name: "Canción", label: 'Canción'},
			{name: "Charteo", label: 'Charteo'},
			{name: "Eventos", label: 'Eventos'},
			{name: "Sección", label: 'Sección'},
		...
		text =
		"W/S o Rueda del Mouse - Cambiar el tiempo del Conductor Strum
		\nA/D - Ir a la Previa/Siguiente sección
		\nLeft/Right - Cambiar Snap
		\nUp/Down - Cambiar el tiempo del Conductor Strum mientras se Snappea
		\nCorchete  Izq. / Corchete  Der.- Cambiar Playback Rate (Shift para ir rápido)
		\nALT + Corchete  Izq. / Corchete  Der - Resetear Playback Rate
		\nMantener Shift para ir 4 veces más rápido
		\nManterner Control + Click Izq. - Selecionar una nota
		\nZ/X - Más/Menos Zoom
		\n
		\nEsc - Probar tu chart dentro del Chart Editor
		\nEnter - Juega tu chart
		\nQ/E - Reducir/Incrementar Largo de una nota
		\nSpace - Para/Resumir Canción";
	...
	function addSongUI():Void
	{
		...
		var check_voices = new FlxUICheckBox(10, 25, null, null, "Tiene voice track", 100);
		...
		var saveButton:FlxButton = new FlxButton(110, 8, "Guardar", function()
		{
			saveLevel();
		});

		var reloadSong:FlxButton = new FlxButton(saveButton.x + 90, saveButton.y, "Recargar Audio", function()
		{
		...
		var reloadSongJson:FlxButton = new FlxButton(reloadSong.x, saveButton.y + 30, "Recargar JSON", function()
		{
			openSubState(new Prompt('Esta acción borrará el progreso actual.\n\n¿Continuar?', 0, function(){loadJson(_song.song.toLowerCase()); }, null,ignoreWarnings));
		});

		var loadAutosaveBtn:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 30, 'Cargar AutSv.', function()
		{
			PlayState.SONG = Song.parseJSONshit(FlxG.save.data.autosave);
			MusicBeatState.resetState();
		});

		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Cargar Evts.', function()
		{
		...
		var saveEvents:FlxButton = new FlxButton(110, reloadSongJson.y, 'Guardar Evts', function ()
		{
			saveEvents();
		});

		var clear_events:FlxButton = new FlxButton(320, 310, 'Borrar eventos', function()
			{
		...
		var clear_notes:FlxButton = new FlxButton(320, clear_events.y + 30, 'Borrar notas', function()
			{
		...
		var reloadNotesButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 20, 'Cambiar Notas', function() {
			_song.arrowSkin = noteSkinInputText.text;
			updateGrid();
		});

		var tab_group_song = new FlxUI(null, UI_box);
		tab_group_song.name = "Canción";
		...
		tab_group_song.add(new FlxText(stepperBPM.x, stepperBPM.y - 15, 0, 'BPM de la canción:'));
		...
		tab_group_song.add(new FlxText(noteSkinInputText.x, noteSkinInputText.y - 15, 0, 'Textura de Notas:'));
		tab_group_song.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 0, 'Textura de Note Splashes:'));
	...
	function addSectionUI():Void
	{
		...
		tab_group_section.name = 'Sección';

		check_mustHitSection = new FlxUICheckBox(10, 15, null, null, "Sección Must hit", 100);
		check_mustHitSection.name = 'check_mustHit';
		check_mustHitSection.checked = _song.notes[curSec].mustHitSection;

		check_gfSection = new FlxUICheckBox(10, check_mustHitSection.y + 22, null, null, "Sección de GF", 100);
		check_gfSection.name = 'check_gf';
		check_gfSection.checked = _song.notes[curSec].gfSection;
		// _song.needsVoices = check_mustHit.checked;

		check_altAnim = new FlxUICheckBox(check_gfSection.x + 120, check_gfSection.y, null, null, "Animación Alterna", 100);
		...
		check_changeBPM = new FlxUICheckBox(10, stepperBeats.y + 30, null, null, 'Cambiar BPM', 100);
		...
		var copyButton:FlxButton = new FlxButton(10, 190, "Copiar Sección", function()
		{
			notesCopied = [];
		...
		var pasteButton:FlxButton = new FlxButton(copyButton.x + 100, copyButton.y, "Pegar Sección", function()
		{
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Borrar", function()
		{
			if(check_notesSec.checked)
			{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Inter. Sec.", function()
		{
			for (i in 0..._song.notes[curSec].sectionNotes.length)
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
		...
		var copyLastButton:FlxButton = new FlxButton(10, swapSection.y + 30, "Copiar Sección ant.", function()
		{
		...
		tab_group_section.add(new FlxText(stepperBeats.x, stepperBeats.y - 15, 0, 'Beats por Sección:'));
	...
	function addNoteUI():Void
	{
		...
		tab_group_note.add(new FlxText(10, 10, 0, 'Largo de Sostenido:'));
		tab_group_note.add(new FlxText(10, 50, 0, 'Tiempo en el Strum (en milisegundos):'));
	...
	function addEventsUI():Void
	{
		...
		selectedEventText = new FlxText(addButton.x - 100, addButton.y + addButton.height + 6, (moveRightButton.x - addButton.x) + 186, 'Evento Selecionado: Ninguno');
	...
	function changeEventSelected(change:Int = 0)
	{
		...
		{
			curEventSelected = 0;
			selectedEventText.text = 'Evento Selecionado: Ninguno';
		}
		updateNoteUI();
	}
	...
	function addChartingUI() {
		...
		waveformUseInstrumental = new FlxUICheckBox(10, 90, null, null, "Waveform para el Instrumental", 100);
		...
		waveformUseVoices = new FlxUICheckBox(waveformUseInstrumental.x + 120, waveformUseInstrumental.y, null, null, "Waveform para las Voces", 100);
		...
		check_mute_inst = new FlxUICheckBox(10, 310, null, null, "Mutear Inst. (en editor)", 100);
		...
		mouseScrollingQuant = new FlxUICheckBox(10, 200, null, null, "Quantific. de Scroll del Mouse", 100);
		...
		check_vortex = new FlxUICheckBox(10, 160, null, null, "Editor Vortex (BETA)", 100);
		...
		check_warnings = new FlxUICheckBox(10, 120, null, null, "Ignorar Advers. de Progreso", 100);
		...
		var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y, null, null, "Mutear Vocales    (en editor)", 100);
		...
		playSoundBf = new FlxUICheckBox(check_mute_inst.x, check_mute_vocals.y + 30, null, null, 'Reproducir Sonido    (Notas de BF)', 100,
			function() {
		...
		playSoundDad = new FlxUICheckBox(check_mute_inst.x + 120, playSoundBf.y, null, null, 'Reproducir Sonido    (Notas de Opo.)', 100,
			function() {
		...
		metronome = new FlxUICheckBox(10, 15, null, null, "Metronómo Activado", 100,
			function() {
		...
		disableAutoScrolling = new FlxUICheckBox(metronome.x + 120, metronome.y, null, null, "Desactivar Autoscroll (No Recomendado)", 120,
			function() {
		...
		tab_group_chart.add(new FlxText(instVolume.x, instVolume.y - 15, 0, 'Volumen del Inst.'));
		tab_group_chart.add(new FlxText(voicesVolume.x, voicesVolume.y - 15, 0, 'Volumen de las Vocales'));
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							if(_song.player1 == 'aquino-gf') { //Easter egg
								soundToPlay = 'AGF_' + Std.string(data + 1);
							}
							else if	(_song.player1 == 'aquinovia') {

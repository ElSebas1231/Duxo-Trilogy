...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Viendo el Menú de Mods", null, null, false, null, 'icon');
		#end
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "NO HAY MODS\nPRESIONA BACK PARA SALIR E INSTALA UN MOD", 48);
		...
		startX -= 190;
		buttonDisableAll = new FlxButton(startX, 0, "DESACT. TODOS", function() {
			for (i in modsList)
			{
				i[1] = false;
			}
		...
		startX -= 190;
		buttonEnableAll = new FlxButton(startX, 0, "ACTIV. TODOS", function() {
			for (i in modsList)
			{
				i[1] = true;
			}
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				if (mod.restart){//finna make it to where if nothing changed then it won't reset
					descriptionTxt.text += " (¡Este Mod reiniciará el juego!)";
				}
